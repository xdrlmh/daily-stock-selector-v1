...
> 完全云端执行 / 零月费 / 永久免费 — 每交易日 **08:35 推送主升浪精选**、**18:30 推送盘后复盘 + 模拟盘移动止盈结算**
...
- ✅ **自动补仓**：止盈/止损平仓后，用当日强势股 TOP 自动补齐到 3 只
...
| 自动补仓 | 平仓后补齐至 3 只 | 用当日强势股 TOP 补位，当日刚平仓的票不回补 |
...
> 举例：买入 10 元 → 涨到 15 元（+50%，峰值）→ 回撤线抬到 14.25 元 → 跌破 14.25 元卖出，锁定约 **+42.5%**；
> 若只涨到 11.5 元（+15%）就回落 → 回撤线 10.925 元 → 跌破即卖出，锁定约 **+9.3%**。
...
│   ├── selector.py                 # 五维评分筛选
│   ├── portfolio.py                # 模拟盘持仓池（移动止盈 / 止损 / 补仓）
