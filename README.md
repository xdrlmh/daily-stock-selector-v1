...
- ✅ **时间止损**：清理僵尸股（≥5 交易日且期间最高 <3%）与低效股（≥10 交易日且 <5%），把仓位让给新机会
...
| 时间止损 | **僵尸 5 日 / 低效 10 日** | 长期不走出趋势 → 清仓让位（详见下节） |
...
| 类型 | 触发条件 | 动作 |
|---|---|---|
| 🧟 **僵尸股** | 持有 ≥ **5 个交易日** 且 期间最高涨幅 < **3%** | 清仓 |
| 🐌 **低效股** | 持有 ≥ **10 个交易日** 且 未启动移动止盈 且 期间最高涨幅 < **5%** | 清仓 |
...
> 🧟 举例：某票 10 元建仓，持有 5 个交易日期间最高只到 10.25（+2.5%）→ 判定僵尸股 → 清仓换股。
> 🐌 举例：某票 20 元建仓，持有 10 个交易日期间最高只到 20.90（+4.5%）→ 判定低效股 → 清仓换股。
