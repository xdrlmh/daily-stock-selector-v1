...
- ✅ **模拟盘持仓**：自动跟进头 3 只，**移动止盈**（+15% 启动，峰值回撤 5% 卖出）＋ 固定止损 -7%
...
|---|---|---|
| 最大持仓 | 3 只 | 满仓不再补仓 |
| 买入价 | 信号次日**开盘价** | 贴近模拟盘真实成本（复盘时归一化） |
| 止损 | **-7%**（固定） | 收盘盈亏 ≤ -7% 平仓 |
| 移动止盈 | **+15% 启动 / 回撤 5% 卖出** | 盈利首次 ≥ +15% 启动，随后跟踪最高价，跌回撤线即卖出 |
...
3. **卖出**：盘中最低价 ≤ `trail_high × (1 - 5%)` → 视为触发，按**触发线价格**成交
